...
def transfer_act(ws, obj):
    ...
    ws['A2'] = f'АКТ СДАЧИ ПРИЁМКИ ОКАЗАННЫХ УСЛУГ № {obj["id"]} от «{obj["date_start_word"][0]}» {obj["date_start_word"][1]} {obj["date_start_word"][2]} года'

    ws['A6'] = f'ООО "Валиуллина", именуемое в дальнейшем "ИСПОЛНИТЕЛЬ", и пациент {obj["patient_name"]} {obj["year"]}' \
               f' года рождения, проживающий (-ая) по адресу {obj["address"]}, именуемый (-ая) в дальнейшем ' \
    ...
    ws['A29'] = f"Исполнитель:" + " " * 109 + "Заказчик:\n\n\n\n" \
                "__________________/Валиуллина Д. И./" + " " * 60 + f"__________________/{obj['patient_name_output']}/\n" \
                + " " * 18 + "М.П."